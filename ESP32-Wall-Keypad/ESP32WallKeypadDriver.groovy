/**
 * IMPORT URL: https://raw.githubusercontent.com/csteele-pd/Hubitat-public/main/ESP32-Wall-Keypad/hubitat/ESP32WallKeypadDriver.groovy
 *
 * ESP32 Wall Keypad Driver
 *
 *
...
metadata {
    definition(name: "ESP32 Wall Keypad", namespace: "keypad", author: "CSteele / OpenAI", importUrl: "https://raw.githubusercontent.com/csteele-pd/Hubitat-public/main/ESP32-Wall-Keypad/hubitat/ESP32WallKeypadDriver.groovy") {
        capability "Actuator"
    ...
    preferences {
        input name: "brokerHost", type: "text", title: "MQTT broker host or URI", required: false
        input name: "brokerPort", type: "text", title: "MQTT broker port", required: false, defaultValue: "1883"
...
void publishState(String armedState, Integer delayRemaining = null) {
    ...
    publishMqtt("state", payload, 1, true)
...
void sendResult(String result, String reason = "", String message = "") {
    ...
    publishMqtt("result", payload, 1, false)
...
void publishDashboard(String dashboardId, String dashboardJson) {
    ...
    publishMqtt("dashboard/${normalizedDashboardId}", payload, 1, true)
...
void clearDashboard(String dashboardId) {
    ...
    publishMqtt("dashboard/${normalizedDashboardId}", "", 1, true)
...
private void publishDeviceCommand(String commandName) {
    ...
    publishMqtt("command", payload, 1, false)
...
private Boolean mqttConfigured() {
    return nullIfBlank(brokerHost) != null
}

private void publishMqtt(String suffix, payload, Integer qos = 1, Boolean retain = false) {
    if (!mqttConfigured()) {
        return
    }

    String body = payload instanceof String ? payload : JsonOutput.toJson(payload)
    try {
        interfaces.mqtt.publish(topicName(suffix), body, qos, retain)
    } catch (Exception e) {
        sendEvent(name: "mqttStatus", value: "error")
        if (logEnable) log.debug "MQTT publish ${suffix} failed: ${e.message}"
    }
}
